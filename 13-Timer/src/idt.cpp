...
extern "C" void interrupt_handler(uint32_t interrupt_number) {
    ...
    else if (interrupt_number == 33) {                  // Keyboard IRQ
    ...
    // --- New Code: Lesson 13 ---
        ...
        // new Timer (IRQ0). We just need to count the tick, then send the end of interrupt signal to the PIC.
        timer_tick();
        pic_send_eoi(0);        // 0 = IRQ0
    }
...
extern "C" {
    ...
    void irq1();   // keyboard IRQ stub
    void irq0();   // New timer IRQ stub
}
...
void idt_init() {
    ...
    idt_set_entry(32, (uint32_t)irq0, 0x08, 0x8E);   // New timer (IRQ0)
