...
// --- New Code Lesson 11 --- (Exception Handler was deleted)
// this single entry point can be called by every stub, and decidedes what kind of interrupt happens based on the number, and then desides what to do.
extern "C" void interrupt_handler(uint32_t interrupt_number) {
    if (interrupt_number < 32) {
         // A CPU exception (0-31) is unrecoverable for our OS at the moment, so we print it and halt.
        print("\n*** CPU EXCEPTION ***\n");
        ...
        // IRQ1 is the keyboard, so we read the scancode from the data port and print the character.
        uint8_t scancode = inb(KEYBOARD_DATA_PORT);
        if (!(scancode & 0x80)) {                 // ignore key-release codes, with the high bit set.
            char c = scancode_to_char[scancode];
            if (c != 0) putchar(c);
        }
        pic_send_eoi(1); // The 1 is the irq number, in this case 1 for the keyboard.
    }         
}

// --- Stubs from idt_stubs.s ---
// --- Changed Code Lesson 11 ---
extern "C" {
...
void idt_init() {
    ...
    idt_set_entry(33, (uint32_t)irq1, 0x08, 0x8E); // New code: Entry for the irq1, in slot 33.
